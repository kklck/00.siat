...
def analyze_html(url, root_url):
    ...
    savepath = Ex07_alldown2.download_file(url)  # 해당 경로를 로컬로 저장
    if savepath is None: return
    if savepath in proc_files: return  # 이미 처리된 파일이면 실행하지 않음
    proc_files[savepath] = True  # 경로명을 key 로 받는 딕셔너리
    print(proc_files)

    # (2) 링크 추출
    f = open(savepath, "r", encoding="utf-8")
    html = f.read()
    links = Ex07_alldown1.enum_links(html, url) # 절대 경로화한 a 태그들의 href 값들
