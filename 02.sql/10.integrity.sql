...
dept : emp
1.공통 컬럼 : deptno
2.어떤 table의 deptno가 종속? : DEPT꺼
	
3.제약사항
	-dept의 deptno
		-unique : 중복 불허
		-not null : 반드시 값이 있어야 함
		-primary key(기본키, pk) : 유니크 + 낫 널
		-용도 : 데이터 구분을 위한 기준 데이터
		
	-emp의 deptno
		-empno : 중복 불허, 낫 널, 프리마리 키
		-deptno : DEPT테이블의 DEPTNO에 종속. 즉, 참조(FOREIGN KEY)
...
2.제약 조건 종류
	...
	-FK[foreign key] : 외래키, 참조하는 자식 테이블, 참조된 키는 부모에서 삭제 불가
...
INSERT INTO EMP02 VALUES(2, 'TESTER', 50);--부모키에는 50이 없다.(참조 확인)
...
drop table dept01;
drop table emp01;
create table dept01 as select * from dept;--제약조건은 복사 X
...
DROP TABLE DEPT01;--EMP01이 외래키로 참조 중이기 때문에 부모 삭제 불가
DROP TABLE DEPT01 CASCADE CONSTRAINT;--제약조건을 무시하고 진행, 자식 테이블의 외래키 제약조건은 삭제됨
...
CREATE TABLE EMP01(
        ENAME VARCHAR2(10),
        AGE NUMBER(3) CONSTRAINT EMP01_AGE_CK CHECK(AGE BETWEEN 1 AND 100)--1~100 사이 값만 오도록 체크
);

INSERT INTO EMP01 VALUES('SIAT', 100);
SELECT * FROM EMP01;
INSERT INTO EMP01 VALUES('SIAT2', 101);--CHECK 에러
...
CREATE TABLE EMP01(
    NAME VARCHAR2(10),
    GENDER CHAR(1) CONSTRAINT EMP01_GENDER_CK CHECK(GENDER = 'M'OR GENDER = 'F')--GENDER IN('M','F')
);

INSERT INTO EMP01 VALUES('SIAT', 'A');
INSERT INTO EMP01 VALUES('SIAT', 'F');
...
INSERT INTO EMP01 VALUES ('SIAT2',' ');
